   ...
   <table align="center" width="100%">
    ...
    <tr>
       <td colspan="7" bgcolor="d3eaef" class="STYLE6" align="center">	
			...
			<form action="<%=basePath%>servlet/ImplQueryServlet" method="post">
			项目名称：
    		...
    		<% 
    		 ...
    		 <tr bgcolor="#FFFFFF" class="STYLE19" class="tb1" align="center">
    			 ...
    			 <td width="20%"><%=str.get(i++) %></td>
    			  <td width="20%"><%if(pro.getPlanCon()==0) {%>等待制定项目实施里程碑计划<%} %>
    			  <%if(pro.getPlanCon()==1) {%>等待领导审批项目实施计划<%} %>
    			  <%if(pro.getPlanCon()==2) {%>项目里程碑计划待修改<%} %>
    			  <%if(pro.getPlanCon()==3) {%>里程碑计划已通过<br/>项目实施中<%} %>
    			  </td>
    			  <td width="20%"><%=pro.getPlanmoney() %></td>
    			  
    			  <td width="20%"><a href="<%=basePath%>servlet/ImplclearServlet?id=<%=pro.getProid() %>&num=<%=pro.getProdep() %>">详细</a>
    			  <a href="<%=basePath%>servlet/ImpltaskServlet?id=<%=pro.getProid() %>">里程碑计划</a>
    			  </td>
    		 </tr>
    		<%			}}  else {%>
    		<tr>
